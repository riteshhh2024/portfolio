...
function ScrollArea({
  ...
  const onMouseEnter = () => {
    if (lenisInstance) {
      // Lenis ≥1.1 uses pause()/resume(); older versions use stop()/start()
      const l = lenisInstance as Lenis & { pause?: () => void };
      if (typeof l.pause === 'function') l.pause();
      else if (typeof (l as Lenis & { stop?: () => void }).stop === 'function')
        (l as Lenis & { stop?: () => void }).stop!();
    }
  };

  const onMouseLeave = () => {
    if (lenisInstance) {
      const l = lenisInstance as Lenis & { resume?: () => void };
      if (typeof l.resume === 'function') l.resume();
      else if (typeof (l as Lenis & { start?: () => void }).start === 'function')
        (l as Lenis & { start?: () => void }).start!();
    }
  };
